...
<body>
    <header>
        ...
        @endauth
        <!-- ACTUALIZADO: Notificacion -->

        <div class="dropdown">
            <i class="far fa-bell fa-3x"></i>
            @if (count(auth()->user()->unreadNotifications))
                <span style="font-size: 1.5rem;"  class="badge badge-warning ">
                    {{ count(auth()->user()->unreadNotifications) }}
                </span>
            @endif
            <div class="dropdown-content">
                <p class="first-p">Notificaciones recientes</p>


                @foreach (auth()->user()->unreadNotifications as $notificacion)
                    <p class="second-p" > {{ $notificacion->data['accion'] }} {{ $notificacion->data['nombre'] }}</p>

                    <hr size="2px" color="black" />

                @endforeach
                <a style="font-size: 1.2rem; " href="{{ route('noticacion.create') }}">Marcar Como Leido</a>
            </div>
        </div>
